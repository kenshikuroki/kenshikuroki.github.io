...
class EnhancedCitationUpdater:
  def __init__(self, json_path="assets/data/publications.json"):
    ...
    self.http.headers.update({
      'User-Agent': 'kenshikuroki.github.io citation updater'
    })
  ...
  def get_paper_by_inspire_id(self, inspire_id: str) -> Optional[Dict]:
    ...
    try:
      response = self.api_get(f"/literature/{inspire_id}")
  ...
  def search_by_arxiv(self, arxiv_id: str) -> Optional[Dict]:
    """arXiv IDで検索"""
    try:
      response = self.api_get('/literature', params={'q': f'eprint:{arxiv_id}', 'size': 1})
      response.raise_for_status()
      data = response.json()
  ...
  def search_by_doi(self, doi: str) -> Optional[Dict]:
    """DOIで検索"""
    try:
      response = self.api_get('/literature', params={'q': f'doi:{doi}', 'size': 1})
      response.raise_for_status()
      data = response.json()
      if data['hits']['total'] > 0:
        return self.extract_paper_metadata(data['hits']['hits'][0])
  ...
  def search_by_title(self, title: str) -> Optional[Dict]:
    ...
    try:
      ...
      response = self.api_get('/literature', params={'q': f'title:"{query_title}"', 'size': 5})
      response.raise_for_status()
      data = response.json()
      # タイトルの類似度をチェック
